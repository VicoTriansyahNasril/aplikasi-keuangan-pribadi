...
export const TransactionProvider = ({ children }) => {
  ...
  const updateTransaction = async (updatedTransaction) => {
    ...
    if (oldTransaction.accountId !== updatedTransaction.accountId || oldTransaction.amount !== updatedTransaction.amount || oldTransaction.type !== updatedTransaction.type) {
  ...
  const addAccount = async (name, initialBalance) => { if (!currentUser) return; const newAccount = { id: uuidv4(), name, balance: initialBalance }; await setDoc(doc(db, 'users', currentUser.uid, 'accounts', newAccount.id), newAccount); setAccounts(prev => [...prev, newAccount]); };
  const deleteAccount = async (id) => { if (!currentUser) return; await deleteDoc(doc(db, 'users', currentUser.uid, 'accounts', id)); setAccounts(prev => prev.filter(acc => acc.id !== id)); };
  const setBudgetForCategory = async (category, amount) => { if (!currentUser) return; const newBudgets = { ...budgets, [category]: amount }; await setDoc(doc(db, 'users', currentUser.uid, 'data', 'budgets'), newBudgets); setBudgets(newBudgets); };
  const addCategory = async (name) => { if (!currentUser || categories.includes(name) || !name) return; const newCategories = [...categories, name]; await setDoc(doc(db, 'users', currentUser.uid, 'data', 'categories'), { list: newCategories }); setCategories(newCategories); };
  const deleteCategory = async (name) => { if (!currentUser) return; const newCategories = categories.filter(c => c !== name); await setDoc(doc(db, 'users', currentUser.uid, 'data', 'categories'), { list: newCategories }); setCategories(newCategories); };
  const addGoal = async (name, targetAmount) => { if (!currentUser) return; const newGoal = { id: uuidv4(), name, targetAmount, currentAmount: 0 }; await setDoc(doc(db, 'users', currentUser.uid, 'goals', newGoal.id), newGoal); setGoals(prev => [...prev, newGoal]); };
  const contributeToGoal = async (goalId, amount, fromAccountId) => { if (!currentUser) return; const goalToUpdate = goals.find(g => g.id === goalId); const newCurrentAmount = goalToUpdate.currentAmount + amount; await addTransaction({ type: 'Pengeluaran', description: `Menabung untuk: ${goalToUpdate.name}`, amount, category: 'Tujuan Menabung', date: new Date().toISOString().split('T')[0], accountId: fromAccountId }, false); await setDoc(doc(db, 'users', currentUser.uid, 'goals', goalId), { currentAmount: newCurrentAmount }, { merge: true }); setGoals(prev => prev.map(g => g.id === goalId ? { ...g, currentAmount: newCurrentAmount } : g)); toast.success('Berhasil menabung untuk tujuan!'); };
  const deleteGoal = async (id) => { if (!currentUser) return; await deleteDoc(doc(db, 'users', currentUser.uid, 'goals', id)); setGoals(prev => prev.filter(g => g.id !== id)); };
  const addRecurring = async (recurringData) => { if (!currentUser) return; const newRecurring = { id: uuidv4(), ...recurringData }; await setDoc(doc(db, 'users', currentUser.uid, 'recurring', newRecurring.id), newRecurring); setRecurring(prev => [...prev, newRecurring]); toast.success('Template berulang ditambahkan!'); };
  const deleteRecurring = async (id) => { if (!currentUser) return; await deleteDoc(doc(db, 'users', currentUser.uid, 'recurring', id)); setRecurring(prev => prev.filter(r => r.id !== id)); };
