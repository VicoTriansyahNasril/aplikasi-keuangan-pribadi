  ...
  const columns = [
    ...
    { header: 'Aksi', accessor: 'actions', render: (row) => (
      <div className={tableStyles.actionCell}>
        ...
        <button className={tableStyles.deleteButton} onClick={() => { if (window.confirm(`Yakin ingin menghapus transaksi "${row.description}"?`)) deleteTransaction(row.id); }} title="Hapus Transaksi"><FaTrash /></button>
      </div>
    )},
  ];
  ...
  return (
    <div>
      <header style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 'var(--spacing-lg)', marginBottom: 'var(--spacing-xl)' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: '700' }}>Riwayat Transaksi</h1>
          <p style={{ color: 'var(--color-text-secondary)' }}>Kelola semua pemasukan dan pengeluaran Anda.</p>
        </div>
      ...
      {transactions.length > 0 ? (
        ...
        <div style={{ backgroundColor: 'var(--color-surface)', borderRadius: 'var(--border-radius-md)'}}>
          <EmptyState
            icon={<FaFileInvoiceDollar />}
            title="Belum Ada Transaksi"
            message="Catat transaksi pertamamu dengan menekan tombol 'Tambah Transaksi' di halaman Dashboard."
          />
        </div>
      )}

      <Modal isOpen={!!editingTransaction} onClose={() => setEditingTransaction(null)} title="Edit Transaksi">
        {editingTransaction && (
          <TransactionForm onClose={() => setEditingTransaction(null)} transactionToEdit={editingTransaction} />
        )}
      </Modal>
    </div>
  );
